...
static zic_t const ZIC32_MIN = -1 - (zic_t) 0x7fffffff;
static zic_t const ZIC32_MAX = 0x7fffffff;
...
struct rule
{
	...
	int r_todisstd;						/* is r_tod standard time? */
	int r_todisut;						/* is r_tod UT? */
	int r_isdst;						/* is this daylight saving time? */
...
#if !defined(HAVE_SYMLINK) || !HAVE_SYMLINK
static ssize_t readlink(const char *restrict file, char *restrict buf, size_t size)
{
	(void)file;
	(void)buf;
	(void)size;
...
static void dolink(const char *fromfield, const char *tofield, int staysymlink);
static int getfields(char *buf, char **array, int arrayelts);
static zic_t gethms(const char *string, const char *errstring);
static zic_t getsave(char *field, int *isdst);
...
static int rulesub(struct rule *rp,
					const char *loyearp, const char *hiyearp,
					const char *typep, const char *monthp, const char *dayp, const char *timep);
...
static int ttisstds[TZ_MAX_TYPES];
static int ttisuts[TZ_MAX_TYPES];
...
static ATTRIBUTE_PURE ptrdiff_t size_product(ptrdiff_t nitems, ptrdiff_t itemsize)
{
#ifdef ckd_mul
	ptrdiff_t product;
...
static void ATTRIBUTE_FORMAT((printf, 1, 0)) verror(const char *const string, va_list args)
{
	 ...
	 */
	if (filenum != 0)
		fprintf(stderr, _("\"%s\", line %" PRIdMAX ": "), filename(filenum), linenum);
	vfprintf(stderr, string, args);
	if (rfilenum != 0)
		fprintf(stderr, _(" (rule from \"%s\", line %" PRIdMAX ")"), filename(rfilenum), rlinenum);
	fprintf(stderr, "\n");
}
...
static void make_links(void)
{
	...
	for (i = 0; i < nalinks; i++)
	{
		...
		if (i == j)
		{
			...
			{
				error(_("\"Link %s %s\" is part of a link cycle"), links[i].l_target, links[i].l_linkname);
				break;
			}
			j = nalinks;
			pass_size = nalinks - i;
		...
		if (noise && i < nlinks)
		{
			if (l)
				...
				warning(_("link %s targeting link %s"), links[i].l_linkname, links[i].l_target);
		}
	}
}
...
static void catch_signals(void)
{
	...
	for (i = 0; i < (int)(sizeof signals / sizeof signals[0]); i++)
	{
		...
		act.sa_flags = 0;
		if (sigaction(signals[i], &act, &act0) == 0 && !(act0.sa_flags & SA_SIGINFO) && act0.sa_handler == SIG_IGN)
		{
...
static int timerange_option(char *timerange)
{
...
static int redundant_time_option(char *opt)
{
	if (*opt == '@')
	{
		intmax_t redundant;
...
static int componentcheck(char const *name, char const *component, char const *component_end)
{
	...
	if (component_len == 0)
	{
		if (!*name)
			...
			error(_(component == name
					...
					: *component_end ? "file name '%s' contains '//'" : "file name '%s' ends with '/'"), name);
		return FALSE;
	}
	...
	if (noise)
	{
		...
		if (component_len_max < component_len)
			warning(_("file name '%s' contains overlength component" " '%.*s...'"), name, component_len_max, component);
	}
	return TRUE;
}
...
static int namecheck(const char *name)
{
	...
	static char const benign[] = "-/_" "abcdefghijklmnopqrstuvwxyz" "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
	...
	for (cp = name; *cp; cp++)
	{
		...
		if (noise && !strchr(benign, c))
		{
			warning((strchr(printable_and_not_benign, c)
					 ? _("file name '%s' contains byte '%c'") : _("file name '%s' contains byte '\\%o'")), name, c);
		}
		if (c == '/')
		{
...
static FILE *open_outfile(char const **outname, char **tempname)
{
	...
	while ((fp = fopen(*outname, fopen_mode)) == NULL)
	{
	...
	if (link_errno != 0)
	{
		...
		if (symlink_errno == 0)
		{
			if (link_errno != ENOTSUP)
				warning(_("symbolic link used because hard link failed: %s"), strerror(link_errno));
		} else
		{
			FILE *fp, *tp;
			int c;
...
static int inputline(FILE *fp, char *buf, ptrdiff_t bufsize)
{
	ptrdiff_t linelen = 0;
	ptrdiff_t ch;
...
static void infile(int fnum, const char *name)
{
	...
	for (num = 1;; ++num)
	{
		...
		if (!inputline(fp, buf, sizeof buf))
			break;
		nfields = getfields(buf, fields, sizeof fields / sizeof *fields);
		if (nfields == 0)
		{
			/* nothing to do */
		} else if (wantcont)
...
static int inzsub(char **fields, int nfields, int iscont)
{
	...
	format_len = (int) strlen(fields[i_format]);
...
static zic_t getleapdatetime(char **fields, int expire_line)
{
	const char *cp;
	const struct lookup *lp;
	zic_t i, j;
	...
	const char *timep)
{
	...
	} else
	{
		...
		if (sscanf(ep, "%d%c", &rp->r_dayofmonth, &xs) != 1 ||
			rp->r_dayofmonth <= 0 || (rp->r_dayofmonth > len_months[1][rp->r_month]))
		{
			error(_("invalid day of month"));
			free(dp);
			return FALSE;
...
static struct timerange limitrange(struct timerange r, zic_t lo, zic_t hi, zic_t const *ats, unsigned char const *types)
{
	...
	while (0 < r.leapbase && ((corr[r.leapbase - 1] < corr[r.leapbase]) != (0 < corr[r.leapbase])))
	{
		r.leapcount++;
		r.leapbase--;
	}
...
static void writezone(const char *const name, const char *const string, char version, int defaulttype)
{
	...
	struct timerange range32;
	struct timerange range64;
	 ...
	 */
	{
		...
		{
			if (toi != 0 &&
				((attypes[fromi].at + utoffs[attypes[toi - 1].type]) <= (attypes[toi - 1].at + utoffs[toi == 1 ? 0 : attypes[toi - 2].type])))
			{
	...
	/* Work around QTBUG-53071 for timestamps less than y2038_boundary - 1,
	   ...
	   long-dead by 2038.  */
	if (WORK_AROUND_QTBUG_53071 && timecnt != 0 && ats[timecnt - 1] < y2038_boundary - 1 && strchr(string, '<'))
	{
	...
	for (pass = 1; pass <= 2; ++pass)
	{
		...
		int pretranstype = -1;
		int thisdefaulttype;
		int locut, hicut, thisleapexpiry;
...
static ptrdiff_t doabbr(char *abbr, const struct zone *zp, const char *letters, int isdst, zic_t save, int doquotes)
{
	char *cp;
	char *slashp;
	ptrdiff_t len;
...
static int stringrule(char *result, const struct rule *const rp, zic_t save, zic_t stdoff)
{
	zic_t tod = rp->r_tod;
	int compat = 0;
...
static void outzone(const struct zone *zpfirst, ptrdiff_t zonecount)
{
	...
	for (i = 0; i < zonecount; ++i)
	{
		...
		zic_t save = 0;
		struct zone const *zp = &zpfirst[i];
		int usestart = i > 0 && (zp - 1)->z_untiltime > min_time;
		...
		*startbuf = '\0';
		...
		if (usestart)
		{
			int isdst = startoff != zp->z_stdoff;
...
static int ciprefix(char const *abbr, char const *word)
{
	do
		if (!*abbr)
			return TRUE;
...
static zic_t rpytime(const struct rule *rp, zic_t wantedy)
{
	...
	dayoff = ((wantedy / YEARSPERREPEAT - y / YEARSPERREPEAT
			   + yrem / YEARSPERREPEAT - (yrem % YEARSPERREPEAT < 0)) * DAYSPERREPEAT);
...
static void mkdirs(const char *argname, int ancestors)
{
	char *name;
	char *cp;
	...
	while (cp && ((cp = strchr(cp, '/')) != NULL || !ancestors))
	{
		if (cp)
			*cp = '\0';
